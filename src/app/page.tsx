...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // Safe check for Supabase client before subscribing
    if (supabase) {
    ...
    <div className="min-h-screen flex flex-col bg-slate-50/50">
      <header className="bg-white/80 backdrop-blur-md border-b border-slate-200 py-4 px-6 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          <nav className="flex items-center gap-8">
            ...
            {user && supabase && (
              <button onClick={() => supabase.auth.signOut()} className="text-xs font-black text-red-500 hover:text-red-700 uppercase tracking-widest">
                Sign Out
              </button>
            )}
          </nav>
      ...
      <main className="flex-1 py-12 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          {!user ? (
            <div className="py-12"><AuthForm /></div>
          ) : (
